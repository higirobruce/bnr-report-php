...
//FINANCLIAL INSTITUTIONS
$query = "SELECT c.cli,c.dev,i.nomrest,c.ncp,sum(c.sdecv) sdecv  FROM CLEARINGUSER.NCP_FINANCIERE P
JOIN prod.BKSLD C ON trim(C.NCP)=trim(P.NCP) 
join prod.bkcli i on i.cli=c.cli
WHERE C.DCO=?  and c.sde>?
group by c.cli,i.nomrest,c.ncp,c.dev
order by 1";
...
if ($stmt->execute(array($dco, 0))){
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $cli4[] = (string)$row['CLI'];
        $dev4[] = (int)$row['DEV'];
        $nomrest4[] = (string)$row['NOMREST'];
        $sdecv4[] = (int)$row['SDECV'];
        $ncp[] = (string)$row['NCP'];       
    }
}
